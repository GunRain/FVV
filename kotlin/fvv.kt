...
class FVVV(
    ...
    fun asInts(default: List<Int> = emptyList()): List<Int> = (asType<List<*>>()?.mapNotNull { it as? Int } ?: default)
    ...
    fun print(type: String = "common"): String {
        ...
        fun printFunc(path: String, node: FVVV, indentLv: Int) {
            ...
            if (node.desc.isNotEmpty() && !isMin && !isNodesc) result.append(" <").append(node.desc.replace(">", "\\>"))
                .append(">")
                                    ...
                                    if (idxChar == ']') {
                                        ...
                                        while (true) {
                                            if (when (txt.getOrNull(idx - pos) ?: break) {
                                                    '<' -> if (!inListDesc) true else (txt.getOrNull(idx - pos - 1) != '\\').also {
                                                        inListDesc = false
                                                    }
